...
#
#     for i in range(3):
#         num.insert((random.randint(0, len(num))), ' ')
#
#     num.insert((random.randint(0, len(num))), '-') if random.random() >= 0.5 else num.insert((random.randint(0,
#             len(num))), random.randint(1, 99))
#     return num
#
# class Card():
#     line_1 = Card_line()
...
from random import randint  # добавляю часть библиотеки


def generate_unique_numbers(count, minbound, maxbound):  # функция генерации списка с естественным рандомом заполнения
    if count > maxbound - minbound + 1:                  # от 1 до 90. Позже по одному элементу будем использовать и
        raise ValueError('Incorrect input parameters')   # и убирать с конца списка
    ret = []
    while len(ret) < count:
...
class Game:
    ...
    def __init__(self):  # при старте игры единожды генерируются карты и список кеглей от 1 до 90 в рандомном порядке.
    ...
    def play_round(self) -> int:
        ...
        keg = self.__kegs.pop()  # используется последняя из списка кегля и удаляется из этого списка
